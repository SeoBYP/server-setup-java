...
3. **측정 기반 병목 제거를 5단계 반복** — URI별 분리 측정과 스레드 덤프로 병목을 매 단계 특정(파티션 키 편중 → 서블릿 스레드 고갈 → 컨슈머 DB 대기 → 요청당 중복 왕복)하고 제거해 **통과 처리량 80 → 800 iter/s (10배)**
...
| **대표 결과** | 스레드 덤프·메트릭으로 병목을 5단계에 걸쳐 특정·제거 → threshold 통과 처리량 **80 → 800 iter/s (10배)**, `RATE=600`에서 **p95 174ms · 실패율 0.00%** (정합성 불변식 전부 유지) |
...
| 4 | 쿠폰 조회 1초 TTL 캐시 · remain 초기화 쿠폰당 1회 · 파티션 12→24 · 동시성 12→24 · Hikari 30→50 | 조사 중 | **800** |
...
| **800** | **780** | **1,480ms** | 2.5% | 0.04% | ✅ **통과** |
| 1000 | 930 | 2,650ms | 7.0% | 0.04% | ❌ p95 초과 |
| 1500 | 1,244 | 2,990ms | 17.0% | 0.02% | ❌ p95 초과 |

**개선 요약 (단계 0 → 4)**

| 지표 | Before | After | 변화 |
|---|---:|---:|---:|
| threshold 통과 처리량 | 80 iter/s | **800 iter/s** | **10배** |
| `RATE=300` p95 | 4,790ms | **51ms** | 94배 |
| `RATE=600` p95 | (도달 불가) | **174ms** | — |
| 최대 달성 처리량 | 90 iter/s | **1,244 iter/s** | 13.8배 |
...
단계 4 측정 종료 시점 기준 (쿠폰 20,000장):

| 검증 항목 | 결과 |
|---|---|
| Redis 발급 18,094 + 잔여 1,906 | **= 20,000** ✅ 정확히 일치 |
| DB `user_coupons` 건수 | **18,094** ✅ Redis와 일치 |
...
> **처리량을 10배 올리는 동안 정합성 불변식은 하나도 깨지지 않았습니다.**
...
| 남은 과제 | 내용 |
|---|---|
| **RATE=1000 이상의 p95 초과 원인 미규명** | 통과 상한 800 iter/s. 그 위에서는 처리량이 계속 늘지만(최대 1,244/s) p95가 2s를 넘음. 남은 후보는 reply 컨슈머 경로(동시성 6), 단일 쿠폰 키에 대한 Redis Lua 직렬화, 단일 MySQL 인스턴스 |
...
- **threshold 통과 상한 ≈ 800 iterations/s** (개선 전 80). `RATE=600`은 p95 174ms로 여유 있게 통과
- 최대 달성 처리량은 1,244 iter/s (RATE=1500 투입 시). 그 구간은 p95가 threshold를 넘음
...
1. **RATE=1000 구간 꼬리 지연 규명** — reply 컨슈머 동시성 상향, Redis Lua 실행 시간 계측,
   부하 중 스레드 덤프로 대기 지점 확인
...
| 성능 | 1초 내 응답, 동시 주문 100건 이상 처리 | ✅ **RATE=600에서 p95 174ms** (통과 상한 800 iter/s) |
