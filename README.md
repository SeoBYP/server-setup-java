...
3. **부하 테스트로 병목을 특정하고 제거** — 포화점을 찾고 URI별 분리 측정으로 원인을 Kafka 파티션 키 편중으로 좁혀, 포화점 **90 → 355 iter/s(3.9배)**, `RATE=300` p95 **4,790ms → 110ms**
...
| **대표 결과** | 부하 테스트로 병목을 Kafka 파티션 키 편중으로 특정·제거 → 포화점 **90 → 355 iter/s(3.9배)**, `RATE=300`에서 **p95 110ms · 실패율 0.00%** (정합성 불변식 전부 유지) |
...
| 2026-07-28 | 6 | 6 | **파티션 키를 `couponId` → `userId`로 변경** (아래) |
...
**결과: 포화점 90 → 355 iter/s (약 3.9배), `RATE=300`에서 p95 4,790ms → 110ms**
([처리량 곡선](#처리량-곡선) 참조)
...
목표 도착률을 단계적으로 올리며 **포화점(knee)** 을 찾고, 병목을 제거한 뒤 다시 측정했습니다.

**Before — 파티션 키 = `couponId`**
...
| 목표 RATE | 달성 iter/s | p95 | `dropped_iterations` | 판정 |
|---:|---:|---:|---:|:---:|
| **300** | **299** | **110ms** | 68 (0.4%) | ✅ **통과** |
| 600 | 358 | 2,510ms | 7,254 (40%) | ❌ 포화 |
| 1000 | 351 | 2,380ms | 19,480 (65%) | ❌ |

**개선 요약**

| 지표 | Before | After | 변화 |
|---|---:|---:|---:|
| 포화점 | 90 iter/s | **355 iter/s** | **약 3.9배** |
| `RATE=300` p95 | 4,790ms | **110ms** | **약 44배** |
| `RATE=300` 평균 | 3,040ms | **37ms** | 약 82배 |
| `RATE=300` 드롭률 | 72% | **0.4%** | — |
| 필요 VU 수 | 500 (상한 소진) | **98** | — |

`RATE=300` 기준 두 threshold(`p95 < 2000ms`, `http_req_failed < 0.05`)를 **모두 통과**합니다.

읽는 법:

- **실패율은 Before/After 전 구간 `http_req_failed = 0.00%`** — 포화 상태에서도
  **응답이 깨지는 것이 아니라 느려질 뿐**이며, 모든 응답이 예상 상태코드(200/409/410/403/504) 안에 있습니다.
- 개선 후에도 355 iter/s에서 실링이 존재합니다. RATE를 600으로 올리든 1000으로 올리든
  달성치는 351~358/s로 고정됩니다.
- `dropped_iterations`를 보지 않으면 이 판정을 할 수 없습니다. k6 진행 표시줄은 72%가 드롭되는
  상황에서도 설정값인 `300.00 iters/s`를 계속 출력하기 때문입니다.

### 병목 위치

Actuator `http.server.requests`로 **URI별 소요 시간을 분리 측정**했습니다 (개선 전, RATE=300 구간).
...
최대값 3,015ms는 Kafka reply 대기 상한에 맞닿아 있습니다.
...
| Kafka Consumer 스레드 부족 | `concurrency`를 6으로 재확인 후 재측정 | ❌ 스레드 6개 기동, 결과 동일 |
| DB 커넥션 풀 고갈 | HikariCP 메트릭 | ❌ 획득 평균 **0.014ms**, 타임아웃 **0건** |
| 쿠폰 경로 고정 지연 | 무부하 단일 요청 5회 | ❌ **19ms** — 고정 지연이 아니라 **적체** |
| **프로듀서 파티션 키 편중** | 키를 `couponId` → `userId`로 변경 후 재측정 | ✅ **원인 확정** — 포화점 90 → 355 iter/s |

**"동시성 6인데 왜 안 빨라지나"가 결정적 단서였습니다.** 파티션 1개는 그룹 내 스레드 1개만
소비하므로, 파티션·동시성을 아무리 늘려도 **키가 한 값으로 고정되면 실질 병렬도는 1**입니다.

### 개선 후 정합성 재검증

`RATE=300` 통과 시점 기준 (쿠폰 20,000장 / 요청 17,932건):

| 검증 항목 | 결과 |
|---|---|
| Redis 잔여 8,086 + 발급 11,914 | **= 20,000** ✅ 정확히 일치 |
| `(user_id, coupon_id)` 중복 | **0건** ✅ |
| 재고 음수 상품 | **0건** ✅ |
| 잔액 음수 지갑 | **0건** ✅ |
| `http_req_failed` | **0.00%** ✅ |

> 발급 11,914건은 사용자 20,000명 풀에서 17,932번 무작위 추출 시의 기대 고유 수
> (약 11,850)와 일치합니다. 나머지는 중복 요청이 정상적으로 409로 수렴한 결과입니다.
> **처리량을 4배 올리면서 정합성 불변식은 하나도 깨지지 않았습니다.**
...
| 남은 과제 | 내용 |
|---|---|
| **355 iter/s 실링의 원인 미규명** | 파티션 키 편중 해소로 90 → 355 iter/s까지 올렸으나, 파티션 6개 기준 기대치(약 6배)에는 못 미침. 남은 후보는 reply 컨슈머 경로, 단일 쿠폰 키(`coupon:{id}:remain`)에 대한 Redis Lua 직렬화, Kafka 라운드트립 고유 지연 |
...
- **처리량 실링 ≈ 355 iterations/s** (개선 전 90). `RATE=300`은 p95 110ms로 여유 있게 통과
- 병목이던 **쿠폰 발급 경로**의 파티션 키 편중은 해소. 주문 경로는 24ms로 원래 여유 있음
- 다만 파티션 6개 기준 기대치(약 6배)에는 못 미쳐, **남은 직렬화 지점이 존재**
...
1. **남은 직렬화 지점 규명** — reply 컨슈머 경로 계측, Redis Lua 실행 시간 측정,
   부하 중 스레드 덤프로 대기 지점 확인 (355 → 이론치에 근접시키기)
...
| 성능 | 1초 내 응답, 동시 주문 100건 이상 처리 | ✅ **RATE=300에서 p95 110ms** (포화점 355 iter/s) |
...
| Kafka Consumer 동시성 | claim 6 / reply 6 | `application.yml` (`app.kafka.consumers.*`) |
