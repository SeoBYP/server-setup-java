...
3. **측정 기반 병목 제거를 6단계 반복** — 스레드 덤프·URI별 분리 측정·프로세스별 CPU로 병목을 매 단계 특정(파티션 키 편중 → 서블릿 스레드 고갈 → 컨슈머 DB 대기 → 요청당 중복 왕복 → 중복 분산락 → **앱이 자기 인프라의 CPU를 잠식**)하고 제거해 **통과 처리량 80 → 1,200 iter/s (15배)**
...
| **대표 결과** | 스레드 덤프·메트릭으로 병목을 6단계에 걸쳐 특정·제거 → threshold 통과 처리량 **80 → 1,200 iter/s (15배)**, `RATE=600`에서 **p95 174ms · 실패율 0.00%** (정합성 불변식 전부 유지) |
...
- **쿠폰 스냅샷을 1초 TTL로 캐시**하므로, 발급 기간을 즉시 중단해야 하는 요구가 생기면
  캐시 무효화 경로가 필요합니다 (현재는 최대 1초 지연)

**동시성 방어선 (분산락 제거 후)**

사용자별 분산락을 제거하고, 아래 3중 방어로 정합성을 보장합니다.

| # | 방어선 | 보장하는 것 |
|---|---|---|
| 1 | **Kafka 파티션 키 = `userId`** | 동일 사용자 요청이 같은 파티션 → 컨슈머 스레드 1개가 순차 처리 |
| 2 | **Redis Lua** (`SISMEMBER` + `DECR` + `SADD`) | 검사·차감·기록의 원자성 (전역) |
| 3 | **`user_coupons` UNIQUE** (`user_id,coupon_id`) / (`request_id`) | 최종 방어선. Kafka 재전송에도 발급 1회 |

> ⚠️ 파티션 키를 `userId` 외의 값으로 되돌리면 1번 방어선이 사라집니다.
> 그 경우 `CouponFacade`의 분산락을 반드시 복구해야 합니다.
...
| 2026-07-29 | 24 | 24 | 사용자별 분산락 제거 (파티션 키가 이미 사용자 단위 직렬화) |
...
**결과: 통과 처리량 80 → 300 iter/s. 이후 비동기 전환·증설·왕복 축소·락 제거로 최종 900 iter/s**
...
| 5 | 중복 분산락 제거 (요청당 Redis 왕복 2회 + 경합 시 20ms 스핀 제거) | **머신 CPU 포화** (전체 98%) | 900 |
| 6 | **CPU 친화도 분할** (앱 8 / k6 4 / 인프라 4) — 코드 변경 없음 | 부하 생성기가 여전히 같은 머신 | **1,200** |
...
| **900** | **863** | **1,940ms** | 3.7% | 0.24% | ✅ **통과 (코어 분할 전 상한)** |
| 1000 | 923 | 2,400ms | 7.7% | 0.24% | ❌ p95 초과 |
| 1500 | 1,280 | 2,510ms | 17.0% | 0.16% | ❌ p95 초과 |

> 이 표는 **CPU 코어 분할 전** 기준입니다. 분할 후에는 RATE=1200까지 통과합니다
> ([RATE=1000 이상 — CPU 자원 분할로 해결](#rate1000-이상--cpu-자원-분할로-해결) 참조).

**개선 요약 (단계 0 → 4)**

| 지표 | Before | After | 변화 |
|---|---:|---:|---:|
| threshold 통과 처리량 | 80 iter/s | **1,200 iter/s** (코어 분할 시) | **15배** |
| `RATE=300` p95 | 4,790ms | **51ms** | 94배 |
| `RATE=600` p95 | (도달 불가) | **174ms** | — |
| 최대 달성 처리량 | 90 iter/s | **1,270 iter/s** | 14.1배 |
...
### RATE=1000 이상 — CPU 자원 분할로 해결

처음에는 "단일 머신 한계"로 결론지었지만, **CPU 친화도로 코어를 분할하니 통과했습니다.**

**측정 결과 (RATE=1000, 동일 코드)**

| 코어 배분 | p95 | 달성 iter/s | threshold |
|---|---:|---:|:---:|
| 분할 없음 (전부 공유) | 2,400ms | 923 | ❌ |
| 앱 12 / k6 4 (인프라 몫 **0**) | 2,070ms | 921 | ❌ |
| **앱 8 / k6 4 / 인프라 4** | **1,920ms** | 935 | ✅ **통과** |

**앱에 코어를 더 준 쪽이 오히려 느렸습니다.**
앱 JVM이 16코어 중 12개를 점유하면 **MySQL·Kafka·Redis(WSL2)가 쓸 코어가 남지 않습니다.**
애플리케이션이 자기 인프라를 굶기고 있었던 것입니다.

앱을 8코어로 제한해 인프라에 4코어를 돌려주자 지연이 내려가고 처리량은 오히려 늘었습니다.

**분할 후 처리량 곡선**

| 목표 RATE | 달성 iter/s | p95 | dropped | threshold |
|---:|---:|---:|---:|:---:|
| 1000 | 935 | **1,920ms** | 6.5% | ✅ 통과 |
| **1200** | **1,080** | **1,950ms** | 10.0% | ✅ **통과 (상한)** |
| 1500 | 1,270 | 2,050ms | 18.4% | ❌ 근소 초과 |

**통과 상한 900 → 1,200 iter/s**로 올랐습니다. 코드 변경 없이 자원 배분만 바꾼 결과입니다.

> **교훈**: 단일 머신에 애플리케이션과 인프라를 함께 올릴 때는
> **애플리케이션이 인프라의 CPU를 빼앗지 않도록 상한을 두는 편이 전체 처리량에 유리**합니다.
> 컨테이너 환경이라면 `cpus` 제한, JVM이라면 친화도나 스레드 풀 상한이 그 역할을 합니다.

여전히 남은 제약: **부하 생성기가 같은 머신에 있습니다**(k6에 4코어 할당).
별도 머신에서 부하를 주면 앱과 인프라가 16코어를 온전히 쓸 수 있으므로 상한은 더 올라갑니다.
...
단계 6 측정 종료 시점 기준 (쿠폰 20,000장):

| 검증 항목 | 결과 |
|---|---|
| Redis 발급 19,580 + 잔여 420 | **= 20,000** ✅ 정확히 일치 |
| DB `user_coupons` 건수 | **19,580** ✅ Redis와 일치 |
...
> **처리량을 15배 올리는 동안 정합성 불변식은 하나도 깨지지 않았습니다.**
...
- **threshold 통과 상한 ≈ 1,200 iterations/s** (개선 전 80, CPU 코어 분할 적용 시). `RATE=600`은 p95 174ms
- 최대 달성 처리량은 1,280 iter/s (RATE=1500 투입 시). 그 구간은 p95가 threshold를 넘음
- **앱이 인프라(MySQL·Kafka·Redis)의 CPU를 잠식하지 않도록 상한을 두는 것이 전체 처리량에 유리**
- 남은 제약은 부하 생성기가 같은 머신에 있다는 점 (k6에 4코어 할당)
...
1. **부하 생성기를 별도 머신으로 분리** 후 재측정 — 현재는 k6가 4코어를 점유하므로 상한이 더 올라갈 여지가 있음
2. 운영 환경에서는 컨테이너 `cpus` 제한으로 앱/인프라 CPU 몫을 명시적으로 분리
