...
3. **측정 기반 병목 제거를 4단계 반복** — URI별 분리 측정과 스레드 덤프로 병목을 매 단계 특정(파티션 키 편중 → 서블릿 스레드 고갈 → 컨슈머 DB 대기)하고 제거해 **통과 처리량 80 → 500 iter/s (6.3배)**
...
| **대표 결과** | 스레드 덤프·메트릭으로 병목을 4단계에 걸쳐 특정·제거 → threshold 통과 처리량 **80 → 500 iter/s (6.3배)**, `RATE=500`에서 **p95 578ms · 실패율 0.00%** (정합성 불변식 전부 유지) |
...
graph TB
    ...
    subgraph COUPON["쿠폰 선착순 - Kafka Request/Reply"]
        API --> KREQ[coupon-claim-requested.v1<br/>파티션 12]
...
**결과: 포화점 90 → 355 iter/s. 이후 비동기 서블릿 전환과 파티션 증설로 최종 550 iter/s**
...
| 단계 | 조치 | 측정된 병목 (근거) | 포화점 | 통과 가능 RATE |
|---|---|---|---:|---:|
| 0 | — | Kafka **파티션 키 편중** → 실질 병렬도 1 | 90/s | 80 |
| 1 | 파티션 키 `couponId` → `userId` | **서블릿 스레드 고갈** (202개 중 196개가 `CompletableFuture$Signaller.block`) | 355/s | 300 |
| 2 | 컨트롤러를 `CompletableFuture` 반환(비동기 서블릿)으로 전환 | **컨슈머 6스레드 DB 대기** (12개 리스너 중 6개가 `FullReadInputStream.readFully`) | 428/s | — |
| 3 | 파티션 6→12 · 컨슈머 동시성 6→12 · HikariCP 20→30 · 응답 조립을 전체 목록 조회 → PK 단건 조회 | 조사 중 (꼬리 지연) | **550/s** | **500** |

**최종 측정 (단계 3)**

| 목표 RATE | 달성 iter/s | p95 | `dropped_iterations` | threshold |
|---:|---:|---:|---:|:---:|
| 400 | 398 | **90ms** | 101 (0.4%) | ✅ 통과 |
| **500** | **495** | **578ms** | 273 (0.9%) | ✅ **통과** |
| 600 | 550 | 2,800ms | 3,009 (8.4%) | ❌ 포화 |

**개선 요약 (단계 0 → 3)**

| 지표 | Before | After | 변화 |
|---|---:|---:|---:|
| threshold 통과 처리량 | 80 iter/s | **500 iter/s** | **6.3배** |
| 포화점 | 90 iter/s | **550 iter/s** | 6.1배 |
| `RATE=300` p95 | 4,790ms | **약 90ms 이하** | 50배 이상 |

읽는 법:

- **실패율은 전 단계·전 구간 `http_req_failed = 0.00%`** — 포화 상태에서도 **응답이 깨지는 것이 아니라
  느려질 뿐**이며, 모든 응답이 예상 상태코드(200/409/410/403/504) 안에 있습니다.
...
단계 3 측정 종료 시점 기준 (쿠폰 20,000장):

| 검증 항목 | 결과 |
|---|---|
| Redis 발급 15,436 + 잔여 4,564 | **= 20,000** ✅ 정확히 일치 |
| DB `user_coupons` 건수 | **15,436** ✅ Redis와 일치 |
...
> **처리량을 6배 올리는 동안 정합성 불변식은 하나도 깨지지 않았습니다.**
...
| 성능 | 1초 내 응답, 동시 주문 100건 이상 처리 | ✅ **RATE=500에서 p95 578ms** (포화점 550 iter/s) |
...
| 항목 | 값 | 출처 |
|---|---|---|
| HikariCP 최대 커넥션 | **30** (connection-timeout 10s, max-lifetime 60s) | `application.yml` |
| Kafka 토픽 파티션 | **12** (`replication-factor=1`) — 3 → 6 → 12 | `CouponKafkaTopicsConfig.java` |
| Kafka Consumer 동시성 | claim **12** / reply 6 | `application.yml` (`app.kafka.consumers.*`) |
